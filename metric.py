...
def match(g_list, p_list):
    g_matched = [0 for i in range(len(g_list))]
    p_matched = [0 for i in range(len(p_list))]
    for i, g in enumerate(g_list):
        for j, p in enumerate(p_list):
            if equal(g, p):
                g_matched[i] = 1
                p_matched[j] = 1
    return g_matched, p_matched
...
class CamRestEvaluator(GenericEvaluator):
    def __init__(self, result_path, data):
    ...
    def run_metrics(self):
        ...
        cons_score, reqs_score = self.slot_metric(data, raw_data=raw_data)
        ...
        print('Informal Slots: ', cons_score)
        print('Requestable Slots: ', reqs_score)
        self._print_dict(self.metric_dict)
        print('%.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f ' % (
            cons_score[0], cons_score[1], cons_score[2], reqs_score[0], reqs_score[1], reqs_score[2], bleu_score,
            match[0],
            success_f1[0]))
        return (cons_score[0], cons_score[1], cons_score[2], reqs_score[0], reqs_score[1], reqs_score[2], bleu_score,
                match[0], success_f1[0])
    ...
    def Lei_extract_request(self, z):
        z = z.split()
        for eos in ['EOS_food', 'EOS_pricerange', 'EOS_area']:
            if eos in z:
                idx = z.index(eos)
                z = z[idx + 1:]
                while 1:
    ...
    def Lei_extract_constraint(self, z):
        ...
        if 'moderately' in constraint:
            constraint.discard('moderately')
            constraint.add('moderate')
        # print(self.entities)
        # return s
        return set(constraint).intersection(self.entities)
        # return set(z).difference(['name', 'address', 'postcode', 'phone', 'area', 'pricerange'])
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                if 'SLOT' in turn['generated_response']:
                    ...
                    gen_cons = self.Lei_extract_constraint(gen_bpsan)
                if 'SLOT' in turn['response']:
                    truth_cons = self.Lei_extract_constraint(turn['bspan'])
            ...
            if not gen_cons:
                gen_bpsan = dial[-1]['generated_bspan']
                gen_cons = self.Lei_extract_constraint(gen_bpsan)
            if truth_cons:
    ...
    def slot_metric(self, data, raw_data=None):
        dials = self.pack_dial(data)
        # find out the last placeholder and see whether that is correct
        # if no such placeholder, see the final turn, because it can be a yes/no question or scheduling dialogue
        total_cons_g_matched = []
        total_cons_t_matched = []
        total_reqs_g_matched = []
        total_reqs_t_matched = []

        for dial_id in dials:
            dial = dials[dial_id]
            for turn_num, turn in enumerate(dial):
                gen_bpsan = turn['generated_bspan']
                gen_cons = list(self.Lei_extract_constraint(gen_bpsan))
                gen_reqs = list(self.Lei_extract_request(gen_bpsan))
                truth_cons = list(self.Lei_extract_constraint(turn['bspan']))
                truth_reqs = list(self.Lei_extract_request(turn['bspan']))

                if gen_reqs != truth_reqs:
                    print(turn['user'])
                    print(gen_reqs, truth_reqs)

                cons_g_matched, cons_t_matched = match(gen_cons, truth_cons)
                total_cons_g_matched += cons_g_matched
                total_cons_t_matched += cons_t_matched

                reqs_g_matched, reqs_t_matched = match(gen_reqs, truth_reqs)
                total_reqs_g_matched += reqs_g_matched
                total_reqs_t_matched += reqs_t_matched

        cons_precision = float(sum(total_cons_g_matched)) / (float(len(total_cons_g_matched)) + 1.e-8)
        cons_recall = float(sum(total_cons_t_matched)) / (float(len(total_cons_t_matched)) + 1.e-8)
        cons_fscore = 2 * cons_precision * cons_recall / (cons_precision + cons_recall + 1.e-8)

        reqs_precision = float(sum(total_reqs_g_matched)) / (float(len(total_reqs_g_matched)) + 1.e-8)
        reqs_recall = float(sum(total_reqs_t_matched)) / (float(len(total_reqs_t_matched)) + 1.e-8)
        reqs_fscore = 2 * reqs_precision * reqs_recall / (reqs_precision + reqs_recall + 1.e-8)

        return (cons_precision, cons_recall, cons_fscore), (reqs_precision, reqs_recall, reqs_fscore)


class KvretEvaluator(GenericEvaluator):
    def __init__(self, result_path, data='test'):
        super().__init__(result_path)
        ...
        raw_json = open('./data/kvret/kvret_' + str(data) + '_public.json')
    ...
    def run_metrics(self):
        ...
        cons_score, reqs_score = self.slot_metric(data, raw_data=None)
    ...
    def _extract_request(self, z):
        ...
        return set(z).intersection(reqs)
    ...
    def Lei_extract_request(self, z):
        ...
        for eos in ['EOS_date', 'EOS_location', 'EOS_weather_attribute', 'EOS_poi_type', 'EOS_distance', 'EOS_event',
                    'EOS_time', 'EOS_agenda', 'EOS_party', 'EOS_room']:
            if eos in z:
                idx = z.index(eos)
                z = z[idx + 1:]
                while 1:
    ...
    def Lei_extract_constraint(self, z):
        ...
        reqs = ['address', 'traffic', 'poi', 'poi_type', 'distance', 'weather', 'temperature', 'weather_attribute',
                'date', 'time', 'location', 'event', 'agenda', 'party', 'room', 'weekly_time', 'forecast']
        s = constraint.difference(junk).difference(en_sws).difference(reqs)
        res = set()
        for item in s:
            if item in junk:
                continue
    ...
    def match_rate_metric(self, data, sub='match', bpsans='./data/kvret/test.bpsan.pkl'):
        ...
        for dial_id in dials:
            ...
            if not truth_cons:
                truth_bpsan = dial[-1]['bspan']
                truth_cons = self.Lei_extract_constraint(truth_bpsan)
            if not gen_cons:
                gen_bpsan = dial[-1]['generated_bspan']
                gen_cons = self.Lei_extract_constraint(gen_bpsan)
